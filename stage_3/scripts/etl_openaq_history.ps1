    ...
    while True:
        ...
        try:
            ...
            if attempt <= RETRY_COUNT:
                ...
                LOG.warning("Request error %s %s (attempt %d/%d): %s; retrying in %.1fs",
                            method, path, attempt, RETRY_COUNT, e, sleep_s)
                time.sleep(sleep_s)
                continue
            raise
        ...
        if _retryable_status(resp.status_code) and attempt <= RETRY_COUNT:
            sleep_s = RETRY_BACKOFF ** (attempt - 1)
            LOG.warning("Retryable HTTP %d for %s %s; retrying in %.1fs",
                        resp.status_code, method, url, sleep_s)
            time.sleep(sleep_s)
            continue

        # Non-retryable or out of attempts
        LOG.error("HTTP %d at %s?%s", resp.status_code, url, (resp.request.body or resp.request.url))
...
def find_nearby_locations(
    ...
    for item in _paged_results("/locations", params, max_pages=math.ceil(max_locations / DEFAULT_LIMIT) + 1):
        locs.append(item)
...
def list_sensors_for_location(location_id: int) -> List[Dict[str, Any]]:
    ...
    for item in _paged_results(f"/locations/{location_id}/sensors", params, max_pages=50):
        sensors.append(item)
    return sensors
...
def normalize_hours_rows(
    ...
    """
    if not rows:
        return pd.DataFrame(columns=["utc_datetime", "parameter", "value", "location_id", "sensor_id"])
...
def long_to_city_wide(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Order columns
    cols = ["city", "utc_datetime"] + sorted([c for c in wide.columns if c not in ("city", "utc_datetime")])
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--date-start", help="Override start datetime (ISO 8601, e.g., 2025-07-01T00:00:00Z)")
...
def main() -> int:
    ...
    if not date_from or not date_to:
        LOG.error("Both date_from and date_to are required (in config or CLI overrides).")
        return 2
    ...
    parameters = [str(p).lower() for p in provider_cfg.get("parameters", DEFAULT_PARAMETERS)]
    ...
    LOG.info(
        ...
        len(cities), date_from, date_to, parameters, radius_m, max_locations
    )
    ...
    for c in cities:
        ...
        if not locs:
            LOG.warning("No OpenAQ locations found near %s (lat=%.4f lon=%.4f)", name, lat, lon)
            continue
