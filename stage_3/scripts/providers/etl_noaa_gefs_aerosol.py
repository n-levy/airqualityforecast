# stage_3/scripts/providers/etl_noaa_gefs_aerosol.py
...
log = logging.getLogger("etl_noaa_gefs_aerosol")
...
def etl_provider(config_path: Path) -> None:
    ...
    if cities:
        log.info("[noaa_gefs_aerosol] Filtering to cities: %s", cities)
        df = df[df["city"].isin(cities)].copy()

    out_raw.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_raw, index=False)
    log.info("[noaa_gefs_aerosol] Wrote raw CSV: %s (rows=%d)", out_raw, len(df))

    df_norm = normalize(df)

    out_parquet.parent.mkdir(parents=True, exist_ok=True)
    df_norm.to_parquet(out_parquet, index=False)
    log.info("[noaa_gefs_aerosol] Wrote processed parquet: %s", out_parquet)
...
if __name__ == "__main__":
    ...
    try:
        ...
        log.exception("ETL NOAA GEFS-Aerosol failed")
        raise
