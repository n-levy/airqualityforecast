#!/usr/bin/env python3
"""
Quick Walk-Forward Validation Results

Fast version with 2 pollutants and 2 models for immediate results.
"""
...
def load_and_prepare_data(data_path: Path) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # Sample every 12 hours for faster processing
    df_sampled = df.iloc[::12].copy().reset_index(drop=True)
...
def quick_walk_forward_validation(
    df: pd.DataFrame, feature_cols: List[str]
) -> pd.DataFrame:
    """Quick walk-forward validation with 2 pollutants and key models."""
    ...
    # Get unique dates for daily walk-forward - use every 60th day for speed
...
def analyze_quick_results(results_df: pd.DataFrame) -> None:
    """Analyze and print the quick validation results."""

    print("\n" + "=" * 80)
    print("QUICK WALK-FORWARD VALIDATION RESULTS")
...
def main():
    ...
    # Run quick walk-forward validation
    log.info("Starting quick walk-forward validation with ALL features...")
    results_df = quick_walk_forward_validation(df, feature_cols)
    ...
    # Save results
    results_path = output_dir / "quick_walk_forward_validation_results.csv"
    results_df.to_csv(results_path, index=False)
    log.info(f"Results saved to {results_path}")

    # Analyze and report results
    analyze_quick_results(results_df)
