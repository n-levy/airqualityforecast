...
$script = Join-Path $root "scripts\providers\etl_noaa_gefs_aerosol.py"
Write-Host "Command:" $py $script "--config" $cfg
& $py $script --config $cfg
if ($LASTEXITCODE -ne 0) { throw "ETL NOAA GEFS-Aerosol (Stage 3) failed." }
Write-Host "ETL NOAA GEFS-Aerosol (Stage 3) finished."
