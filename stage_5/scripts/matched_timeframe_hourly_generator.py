...
class MatchedTimeframeHourlyGenerator:
    ...
    def load_data(self):
        ...
        safe_print(
            f"Loaded {len(self.cities_df)} cities for matched timeframe analysis"
        )
        return True
    ...
    def generate_matched_hourly_data(self, city_name):
        ...
        safe_print(
            f"Generating {len(timestamps):,} hours for {city_name} ({self.start_date.date()} to {self.end_date.date()})"
        )
    ...
    def generate_full_matched_dataset(self):
        ...
        safe_print(
            f"Expected records: {self.total_hours * 100:,} (100 cities × {self.total_hours:,} hours)"
        )
        safe_print("=" * 80)
        ...
        for idx, city in enumerate(self.cities_df["City"]):
            try:
                ...
                if city_hourly_data and len(city_hourly_data) > 0:
                    ...
                    if (idx + 1) % 10 == 0:
                        safe_print(
                            f"✅ Progress: {idx+1}/100 cities completed, {total_records:,} records generated"
                        )
                        safe_print(
                            f"   Expected final size: ~{total_records * 0.8 / 1000:.0f} MB so far"
                        )
                else:
        ...
        safe_print(
            f"✅ Expected vs actual: {self.total_hours * 100:,} expected, {total_records:,} actual"
        )
        safe_print(
            f"✅ Records per city: {total_records // successful_cities if successful_cities > 0 else 0:,}"
        )
    ...
    def perform_model_evaluation_sample(self):
        ...
        for city_name in sample_cities:
            ...
            try:
                ...
                test_data = df.iloc[
                    min_train_size : min_train_size + 500
                ]  # Test on 500 hours
    ...
    def save_matched_dataset(self):
        ...
        hourly_file = (
            Path("..")
            / "final_dataset"
            / f"MATCHED_timeframe_hourly_dataset_{timestamp}.json"
        )

        safe_print(f"\n💾 SAVING MATCHED TIMEFRAME DATASET...")
        safe_print(f"File: {hourly_file}")
        safe_print(
            f"Expected size: ~{sum(len(city_data) for city_data in self.hourly_data.values()) * 0.8 / 1000:.0f} MB"
        )
        ...
        results = {
            ...
            "dataset_characteristics": {
                ...
                "hours_per_city": (
                    total_records // len(self.hourly_data) if self.hourly_data else 0
                ),
            ...
            "file_locations": {
                ...
                "analysis_results": str(
                    Path("..")
                    / "final_dataset"
                    / f"MATCHED_timeframe_analysis_{timestamp}.json"
                ),
            },
        }
        ...
        safe_print(
            f"🔍 Size ratio vs daily: {file_size_mb / 14:.1f}x ({file_size_mb:.1f} MB vs 14 MB)"
        )
        safe_print(
            f"📈 Record ratio vs daily: {total_records / 33500:.1f}x ({total_records:,} vs 33,500)"
        )
...
def main():
    ...
    try:
        ...
        safe_print(
            f"✅ File size ratio: {results['size_comparison']['file_size_ratio']}"
        )
