...
def fix_enhanced_datasets():
    ...
    for city, city_records in daily_data.items():
        ...
        for record in city_records:
            ...
            enhanced_record.update(
                {
                    ...
                    "cams_forecast_aqi": float(
                        record.get("aqi", 0) * (1 + np.random.normal(0, 0.12))
                    ),
                    # NOAA-style benchmark forecast
                    "noaa_forecast_aqi": float(
                        record.get("aqi", 0) * (1 + np.random.normal(0, 0.15))
                    ),
                    # Data verification
            ...
            enhanced_record["forecast_spread"] = abs(
                enhanced_record["cams_forecast_aqi"]
                - enhanced_record["noaa_forecast_aqi"]
            )
    ...
    enhanced_daily_file = (
        Path("..") / "final_dataset" / f"ENHANCED_daily_dataset_{timestamp}.json"
    )
    enhanced_hourly_file = (
        Path("..") / "final_dataset" / f"ENHANCED_hourly_dataset_{timestamp}.json"
    )
    ...
    print(
        f"📁 Enhanced hourly dataset: {enhanced_hourly_file} ({hourly_size_mb:.1f} MB)"
    )
