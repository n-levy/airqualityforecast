...
def comprehensive_performance_analysis(results):
            ...
            if pollutant in city_data['results']:
                ...
                for method in methods:
                    if method in pollutant_data:
                        ...
                        row[f'{method}_RMSE'] = pollutant_data[method].get('RMSE', np.nan)
    ...
    df['ridge_vs_cams_mae_improvement'] = ((df['cams_MAE'] - df['ridge_MAE']) / df['cams_MAE'] * 100)
    df['ridge_vs_noaa_mae_improvement'] = ((df['noaa_MAE'] - df['ridge_MAE']) / df['noaa_MAE'] * 100)
    df['ridge_vs_best_benchmark_improvement'] = np.maximum(df['ridge_vs_cams_mae_improvement'], df['ridge_vs_noaa_mae_improvement'])
...
def analyze_continental_performance(df):
    ...
    for continent in df['Continent'].unique():
        ...
        summary = {
            ...
            'Ridge_vs_CAMS_Improvement': continent_data['ridge_vs_cams_mae_improvement'].mean(),
            'Ridge_vs_NOAA_Improvement': continent_data['ridge_vs_noaa_mae_improvement'].mean(),
            'Ridge_vs_Best_Improvement': continent_data['ridge_vs_best_benchmark_improvement'].mean(),
    ...
    print(f"{'Continent':<15} {'Cities':<8} {'Ridge MAE':<10} {'CAMS MAE':<10} {'NOAA MAE':<10} {'Best Improve%':<12}")
    print("-" * 75)
    
    for _, row in continental_df.iterrows():
        print(f"{row['Continent']:<15} {row['Cities']:<8} {row['Ridge_MAE_Avg']:<10.3f} {row['CAMS_MAE_Avg']:<10.3f} {row['NOAA_MAE_Avg']:<10.3f} {row['Ridge_vs_Best_Improvement']:<12.1f}")
...
def analyze_pollutant_performance(df):
    ...
    for pollutant in df['Pollutant'].unique():
        ...
        summary = {
            ...
            'Ridge_vs_CAMS_Improvement': pollutant_data['ridge_vs_cams_mae_improvement'].mean(),
            'Ridge_vs_NOAA_Improvement': pollutant_data['ridge_vs_noaa_mae_improvement'].mean(),
            'Ridge_vs_Best_Improvement': pollutant_data['ridge_vs_best_benchmark_improvement'].mean(),
    ...
    print(f"{'Pollutant':<10} {'Cities':<8} {'Ridge MAE':<10} {'CAMS MAE':<10} {'NOAA MAE':<10} {'Best Improve%':<12}")
    print("-" * 70)
    
    for _, row in pollutant_df.iterrows():
        print(f"{row['Pollutant']:<10} {row['Cities']:<8} {row['Ridge_MAE_Avg']:<10.3f} {row['CAMS_MAE_Avg']:<10.3f} {row['NOAA_MAE_Avg']:<10.3f} {row['Ridge_vs_Best_Improvement']:<12.1f}")
...
def analyze_health_warnings_enhanced(results):
                ...
                if method in health_data:
                    for alert_type in ['sensitive', 'general']:
                        if alert_type in health_data[method]:
                            ...
                            global_totals[method][alert_type]['tp'] += alert.get('true_positives', 0)
                            global_totals[method][alert_type]['fp'] += alert.get('false_positives', 0)
                            global_totals[method][alert_type]['tn'] += alert.get('true_negatives', 0)
                            global_totals[method][alert_type]['fn'] += alert.get('false_negatives', 0)
    
    # Calculate and display metrics
    print(f"{'Method':<8} {'Alert Type':<12} {'Precision':<10} {'Recall':<8} {'FNR':<8} {'Rating':<12}")
    print("-" * 68)
    ...
    for method in methods:
        ...
        for alert_type in ['sensitive', 'general']:
            ...
            fnr = stats['fn'] / (stats['tp'] + stats['fn']) if (stats['tp'] + stats['fn']) > 0 else 0
            ...
            print(f"{method.upper():<8} {alert_type:<12} {precision:<10.3f} {recall:<8.3f} {fnr:<8.3f} {rating:<12}")
...
def generate_summary_report(df, continental_df, pollutant_df, health_ratings):
    ...
    print(f"Overall Ridge Improvement vs Best Benchmark: {overall_ridge_improvement:.1f}%")
    
    # Best and worst performing continents for Ridge
    best_continent = continental_df.loc[continental_df['Ridge_vs_Best_Improvement'].idxmax()]
    worst_continent = continental_df.loc[continental_df['Ridge_vs_Best_Improvement'].idxmin()]
    
    print(f"Best Ridge Performance: {best_continent['Continent']} ({best_continent['Ridge_vs_Best_Improvement']:.1f}% improvement)")
    print(f"Worst Ridge Performance: {worst_continent['Continent']} ({worst_continent['Ridge_vs_Best_Improvement']:.1f}% improvement)")
    
    # Best and worst performing pollutants for Ridge
    best_pollutant = pollutant_df.loc[pollutant_df['Ridge_vs_Best_Improvement'].idxmax()]
    worst_pollutant = pollutant_df.loc[pollutant_df['Ridge_vs_Best_Improvement'].idxmin()]
    
    print(f"Best Pollutant Performance: {best_pollutant['Pollutant']} ({best_pollutant['Ridge_vs_Best_Improvement']:.1f}% improvement)")
    print(f"Worst Pollutant Performance: {worst_pollutant['Pollutant']} ({worst_pollutant['Ridge_vs_Best_Improvement']:.1f}% improvement)")
    
    # Health warning performance
    ridge_avg_fnr = (health_ratings['ridge']['sensitive']['fnr'] + health_ratings['ridge']['general']['fnr']) / 2
    cams_avg_fnr = (health_ratings['cams']['sensitive']['fnr'] + health_ratings['cams']['general']['fnr']) / 2
    noaa_avg_fnr = (health_ratings['noaa']['sensitive']['fnr'] + health_ratings['noaa']['general']['fnr']) / 2
    
    print(f"Health Warning False Negative Rates:")
    print(f"  Ridge: {ridge_avg_fnr:.1%} | CAMS: {cams_avg_fnr:.1%} | NOAA: {noaa_avg_fnr:.1%}")
...
def save_enhanced_analysis_results(df, continental_df, pollutant_df, health_ratings, summary):
    ...
    # Save CSV summaries
    continental_df.to_csv(f"../final_dataset/continental_performance_{timestamp}.csv", index=False)
    pollutant_df.to_csv(f"../final_dataset/pollutant_performance_{timestamp}.csv", index=False)
...
def main():
    ...
    # Save results
    output_file = save_enhanced_analysis_results(df, continental_df, pollutant_df, health_ratings, summary)
    
    print(f"\nFINAL SUMMARY:")
    print(f"✓ Enhanced evaluation analysis completed for {len(results)} cities")
    print(f"✓ Ridge regression shows {summary['overall_improvement']:.1f}% average improvement")
    print(f"✓ Best performance in {summary['best_continent']} for {summary['best_pollutant']}")
