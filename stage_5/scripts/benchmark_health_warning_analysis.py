...
def analyze_benchmark_health_warnings():
                ...
                if method in health_data:
                    for alert_type in ['sensitive', 'general']:
                        if alert_type in health_data[method]:
                            ...
                            continental_totals[continent][method][alert_type]['tp'] += alert.get('true_positives', 0)
                            continental_totals[continent][method][alert_type]['fp'] += alert.get('false_positives', 0)
                            continental_totals[continent][method][alert_type]['tn'] += alert.get('true_negatives', 0)
                            continental_totals[continent][method][alert_type]['fn'] += alert.get('false_negatives', 0)
    
    # Display global performance
    print("\nGLOBAL HEALTH WARNING PERFORMANCE COMPARISON:")
    print(f"{'Method':<8} {'Alert Type':<12} {'Precision':<10} {'Recall':<8} {'FPR':<8} {'FNR':<8} {'F1':<8} {'Rating':<12}")
    print("-" * 88)
    ...
    for method in methods:
        ...
        for alert_type in ['sensitive', 'general']:
            ...
            print(f"{method.upper():<8} {alert_type:<12} {precision:<10.3f} {recall:<8.3f} {fpr:<8.3f} {fnr:<8.3f} {f1:<8.3f} {rating:<12}")
    
    # Benchmark comparison
    print(f"\nBENCHMARK PERFORMANCE ASSESSMENT:")
    print(f"{'Metric':<25} {'Ridge':<12} {'CAMS':<12} {'NOAA':<12} {'Best Benchmark':<15}")
    print("-" * 80)
    ...
    for alert_type in ['sensitive', 'general']:
        ...
        print(f"{alert_type.title() + ' FNR (lower better)':<25} {ridge_fnr:<12.3f} {cams_fnr:<12.3f} {noaa_fnr:<12.3f} {best_benchmark_fnr:<15.3f}")
        print(f"{alert_type.title() + ' FPR (lower better)':<25} {ridge_fpr:<12.3f} {cams_fpr:<12.3f} {noaa_fpr:<12.3f} {best_benchmark_fpr:<15.3f}")
        print(f"{alert_type.title() + ' Precision (higher better)':<25} {ridge_prec:<12.3f} {cams_prec:<12.3f} {noaa_prec:<12.3f} {best_benchmark_prec:<15.3f}")
        print()
    
    # Continental performance for each method
    print("BENCHMARK PERFORMANCE BY CONTINENT:")
    print(f"{'Continent':<15} {'Method':<8} {'Sensitive FNR':<14} {'General FNR':<12} {'Sensitive FPR':<14} {'General FPR':<12} {'Rating':<12}")
    print("-" * 98)
    ...
    for continent in continents:
        ...
        for method in methods:
            ...
            sens_fnr = sens_stats['fn'] / (sens_stats['tp'] + sens_stats['fn']) if (sens_stats['tp'] + sens_stats['fn']) > 0 else 0
            gen_fnr = gen_stats['fn'] / (gen_stats['tp'] + gen_stats['fn']) if (gen_stats['tp'] + gen_stats['fn']) > 0 else 0
            
            sens_fpr = sens_stats['fp'] / (sens_stats['fp'] + sens_stats['tn']) if (sens_stats['fp'] + sens_stats['tn']) > 0 else 0
            gen_fpr = gen_stats['fp'] / (gen_stats['fp'] + gen_stats['tn']) if (gen_stats['fp'] + gen_stats['tn']) > 0 else 0
            ...
            print(f"{'':15} {method.upper():<8} {sens_fnr:<14.3f} {gen_fnr:<12.3f} {sens_fpr:<14.3f} {gen_fpr:<12.3f} {rating:<12}")
    
    # Health impact assessment
    print(f"\nHEALTH IMPACT ASSESSMENT:")
    print(f"{'Method':<8} {'Sensitive Pop.':<15} {'General Pop.':<15} {'Overall Assessment':<20}")
    print("-" * 65)
    ...
    for method in methods:
        ...
        print(f"{method.upper():<8} {sens_rating:<15} {gen_rating:<15} {assessment:<20}")
    ...
    print(f"  Ridge: {ridge_sens_fnr:.1%} | CAMS: {cams_sens_fnr:.1%} | NOAA: {noaa_sens_fnr:.1%}")
    
    print(f"General Population False Negative Rates:")
    print(f"  Ridge: {ridge_gen_fnr:.1%} | CAMS: {cams_gen_fnr:.1%} | NOAA: {noaa_gen_fnr:.1%}")
    ...
    if ridge_total_fnr <= cams_total_fnr and ridge_total_fnr <= noaa_total_fnr:
        ...
        advantage = min((cams_total_fnr - ridge_total_fnr) / ridge_total_fnr, (noaa_total_fnr - ridge_total_fnr) / ridge_total_fnr) * 100
