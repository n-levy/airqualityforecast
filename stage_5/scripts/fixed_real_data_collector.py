...
def continue_waqi_collection():
    ...
    if collection_files:
        ...
        existing_results = {
            ...
            'waqi_data': {},
            'openweather_data': {},
            'success_counts': {'noaa_success': 0, 'waqi_success': 0, 'openweather_success': 0}
        }
    ...
    for idx, row in remaining_cities.iterrows():
        ...
        try:
            ...
            display_name = f"{city_name.encode('ascii', 'replace').decode('ascii')}, {country}"
            print(f"  Collecting WAQI data for {display_name}...")
        
        try:
            # Try multiple query formats
            city_queries = [
                city_name.lower().replace(' ', '-').replace("'", "").replace('ł', 'l').replace('ć', 'c').replace('ń', 'n'),
                city_name.lower().replace(' ', '').replace("'", "").replace('ł', 'l').replace('ć', 'c').replace('ń', 'n'),
    ...
    cities_with_real_data.update([
        city for city, data in existing_results['noaa_data'].items() 
        if data.get('api_status') == 'SUCCESS'
    ])
    
    # Add WAQI cities  
    cities_with_real_data.update([
        city for city, data in existing_results['waqi_data'].items() 
        if data.get('api_status') == 'SUCCESS'
    ])
    ...
    output_file = f"../final_dataset/comprehensive_real_data_collection_fixed_{timestamp}.json"
