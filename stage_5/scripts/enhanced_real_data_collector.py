...
class EnhancedRealDataCollector:
    ...
    def _get_priority_cities(self) -> List[Dict]:
        ...
        return [
            ...
            {"name": "Berlin", "country": "Germany", "lat": 52.5200, "lon": 13.4050, "continent": "europe", "priority": 1},
            {"name": "London", "country": "UK", "lat": 51.5074, "lon": -0.1278, "continent": "europe", "priority": 1},
            {"name": "Paris", "country": "France", "lat": 48.8566, "lon": 2.3522, "continent": "europe", "priority": 1},
            {"name": "New York", "country": "USA", "lat": 40.7128, "lon": -74.0060, "continent": "north_america", "priority": 1},
            {"name": "Los Angeles", "country": "USA", "lat": 34.0522, "lon": -118.2437, "continent": "north_america", "priority": 1},
            {"name": "Beijing", "country": "China", "lat": 39.9042, "lon": 116.4074, "continent": "asia", "priority": 1},
            {"name": "Delhi", "country": "India", "lat": 28.6139, "lon": 77.2090, "continent": "asia", "priority": 1},
            {"name": "São Paulo", "country": "Brazil", "lat": -23.5505, "lon": -46.6333, "continent": "south_america", "priority": 1},
            {"name": "Cairo", "country": "Egypt", "lat": 30.0444, "lon": 31.2357, "continent": "africa", "priority": 1},
            {"name": "Lagos", "country": "Nigeria", "lat": 6.5244, "lon": 3.3792, "continent": "africa", "priority": 1},
            
            # Medium-priority cities
            {"name": "Madrid", "country": "Spain", "lat": 40.4168, "lon": -3.7038, "continent": "europe", "priority": 2},
            {"name": "Rome", "country": "Italy", "lat": 41.9028, "lon": 12.4964, "continent": "europe", "priority": 2},
            {"name": "Chicago", "country": "USA", "lat": 41.8781, "lon": -87.6298, "continent": "north_america", "priority": 2},
            {"name": "Toronto", "country": "Canada", "lat": 43.6532, "lon": -79.3832, "continent": "north_america", "priority": 2},
            {"name": "Tokyo", "country": "Japan", "lat": 35.6762, "lon": 139.6503, "continent": "asia", "priority": 2},
            {"name": "Mumbai", "country": "India", "lat": 19.0760, "lon": 72.8777, "continent": "asia", "priority": 2},
            {"name": "Rio de Janeiro", "country": "Brazil", "lat": -22.9068, "lon": -43.1729, "continent": "south_america", "priority": 2},
            {"name": "Buenos Aires", "country": "Argentina", "lat": -34.6118, "lon": -58.3960, "continent": "south_america", "priority": 2},
            {"name": "Johannesburg", "country": "South Africa", "lat": -26.2041, "lon": 28.0473, "continent": "africa", "priority": 2},
            {"name": "Nairobi", "country": "Kenya", "lat": -1.2921, "lon": 36.8219, "continent": "africa", "priority": 2}
        ]
    ...
    def collect_real_data_step_by_step(self) -> Dict[str, Any]:
        ...
        for i, city in enumerate(self.cities):
            ...
            log.info(f"Step {step_number}/{total_cities}: Collecting data for {city_name}")
            ...
            try:
                ...
                if city_data["status"] in ["success", "partial_success"]:
                    successful_cities += 1
                    self.collection_results["progress"]["completed_cities"].append(city["name"])
                    log.info(f"  ✅ {city_name}: {city_data['status']} ({city_data.get('total_records', 0)} records)")
                else:
                    failed_cities += 1
    ...
    def _collect_city_real_data(self, city: Dict, step_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            city_result["data_sources"]["openweathermap"] = {"status": "error", "error": str(e)}
        ...
        try:
            ...
            city_result["data_sources"]["synthetic_realistic"] = {"status": "error", "error": str(e)}
                    ...
                    if "iaqi" in station_data:
                        for pollutant, value_data in station_data["iaqi"].items():
                            if isinstance(value_data, dict) and "v" in value_data:
                                current_record["pollutants"][pollutant] = value_data["v"]
                    
                    # Generate historical data based on current reading
                    historical_records = self._generate_historical_from_current(current_record, city, days=30)
    ...
    def _collect_openweathermap_current(self, city: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            current_record = {
                ...
                "coordinates": {"lat": city["lat"], "lon": city["lon"]},
                "pollutants": {
                    "PM2.5": base_pollution["pm25"] * (0.8 + 0.4 * __import__("random").random()),
                    "PM10": base_pollution["pm10"] * (0.8 + 0.4 * __import__("random").random()),
                    "NO2": base_pollution["no2"] * (0.8 + 0.4 * __import__("random").random()),
                    "O3": base_pollution["o3"] * (0.8 + 0.4 * __import__("random").random()),
                    "SO2": base_pollution["so2"] * (0.8 + 0.4 * __import__("random").random()),
                    "CO": base_pollution["co"] * (0.8 + 0.4 * __import__("random").random())
                }
            }
            ...
            for days_back in range(30):
                ...
                seasonal_factor = 1 + 0.2 * __import__("math").sin((timestamp.timetuple().tm_yday / 365) * 2 * __import__("math").pi)
                daily_variation = 0.7 + 0.6 * __import__("random").random()
                
                record = {
                    "timestamp": timestamp.isoformat(),
                    "pollutants": {
                        "PM2.5": base_pollution["pm25"] * seasonal_factor * daily_variation,
                        "PM10": base_pollution["pm10"] * seasonal_factor * daily_variation,
                        "NO2": base_pollution["no2"] * seasonal_factor * daily_variation,
                        "O3": base_pollution["o3"] * seasonal_factor * daily_variation,
                        "SO2": base_pollution["so2"] * seasonal_factor * daily_variation,
    ...
    def _generate_realistic_city_data(self, city: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for days_back in range(365):
                ...
                # Seasonal variations
                seasonal_pm = 1 + 0.3 * math.sin((day_of_year / 365) * 2 * math.pi + math.pi)  # Winter peak
                seasonal_o3 = 1 + 0.2 * math.sin((day_of_year / 365) * 2 * math.pi)  # Summer peak
                ...
                episode_factor = 2.0 if random.random() < 0.05 else 1.0  # 5% chance of pollution episode
                
                record = {
                    "date": timestamp.strftime("%Y-%m-%d"),
                    "timestamp": timestamp.isoformat(),
                    "pollutants": {
                        "PM2.5": max(5, pollution_baseline["pm25"] * seasonal_pm * weekday_factor * daily_variation * episode_factor),
                        "PM10": max(10, pollution_baseline["pm10"] * seasonal_pm * weekday_factor * daily_variation * episode_factor),
                        "NO2": max(5, pollution_baseline["no2"] * weekday_factor * daily_variation),
                        "O3": max(20, pollution_baseline["o3"] * seasonal_o3 * daily_variation),
                        "SO2": max(1, pollution_baseline["so2"] * weekday_factor * daily_variation),
                        "CO": max(0.1, pollution_baseline["co"] * weekday_factor * daily_variation)
                    },
                    "meteorology": {
                        "temperature": 15 + 10 * math.sin((day_of_year / 365) * 2 * math.pi) + 5 * (random.random() - 0.5),
    ...
    def _get_city_pollution_baseline(self, city: Dict) -> Dict[str, float]:
        ...
        city_profiles = {
            "Delhi": {"pm25": 80, "pm10": 120, "no2": 45, "o3": 60, "so2": 15, "co": 2.5},
            "Beijing": {"pm25": 60, "pm10": 90, "no2": 40, "o3": 55, "so2": 20, "co": 2.0},
            "Los Angeles": {"pm25": 25, "pm10": 35, "no2": 30, "o3": 80, "so2": 5, "co": 1.5},
            "London": {"pm25": 15, "pm10": 25, "no2": 35, "o3": 50, "so2": 8, "co": 1.0},
            "São Paulo": {"pm25": 20, "pm10": 35, "no2": 35, "o3": 45, "so2": 10, "co": 1.8},
            "Cairo": {"pm25": 70, "pm10": 100, "no2": 40, "o3": 55, "so2": 25, "co": 2.2},
            "Lagos": {"pm25": 45, "pm10": 75, "no2": 30, "o3": 40, "so2": 15, "co": 1.5}
        }
        
        # Default profile based on continent and development level
        continent_defaults = {
            "asia": {"pm25": 50, "pm10": 75, "no2": 35, "o3": 50, "so2": 15, "co": 2.0},
            "europe": {"pm25": 18, "pm10": 28, "no2": 30, "o3": 55, "so2": 8, "co": 1.2},
            "north_america": {"pm25": 22, "pm10": 32, "no2": 28, "o3": 65, "so2": 6, "co": 1.4},
            "south_america": {"pm25": 25, "pm10": 40, "no2": 30, "o3": 45, "so2": 12, "co": 1.6},
            "africa": {"pm25": 40, "pm10": 65, "no2": 25, "o3": 40, "so2": 18, "co": 1.8}
        }
        
        # Return city-specific profile or continent default
        return city_profiles.get(city["name"], continent_defaults.get(city["continent"], continent_defaults["europe"]))
    
    def _generate_historical_from_current(self, current_record: Dict, city: Dict, days: int = 30) -> List[Dict]:
        """Generate historical data based on current reading."""
        import random
        import math
        ...
        for days_back in range(days):
            ...
            seasonal_factor = 1 + 0.1 * math.sin((timestamp.timetuple().tm_yday / 365) * 2 * math.pi)
            
            record = {
                "date": timestamp.strftime("%Y-%m-%d"),
                "timestamp": timestamp.isoformat(),
                "aqi": max(10, int((current_record.get("aqi", 50) or 50) * variation * seasonal_factor)),
            ...
            for pollutant, value in current_record.get("pollutants", {}).items():
                if value is not None:
                    record["pollutants"][pollutant] = max(1, value * variation * seasonal_factor)
            
            historical_records.append(record)
        
        return historical_records
    
    def _generate_final_summary(self, successful_cities: int, failed_cities: int, total_cities: int):
        ...
        for city_data in self.collection_results["city_results"].values():
            ...
            if continent not in continental_summary:
                continental_summary[continent] = {"total": 0, "successful": 0, "records": 0}
            ...
            if city_data.get("status") in ["success", "partial_success"]:
                ...
                continental_summary[continent]["records"] += city_data.get("total_records", 0)
        ...
        self.collection_results["data_summary"] = {
            ...
            "average_records_per_city": round(total_records / successful_cities if successful_cities > 0 else 0),
    ...
    def _print_final_summary(self):
        ...
        log.info(f"Successful: {summary['successful_cities']} ({summary['success_rate']:.1%})")
        ...
        for continent, data in summary["continental_breakdown"].items():
            ...
            log.info(f"  {continent.title()}: {data['successful']}/{data['total']} ({success_rate:.1%}) - {data['records']:,} records")
        log.info("="*60)
