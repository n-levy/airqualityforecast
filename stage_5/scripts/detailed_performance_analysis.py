...
class DetailedPerformanceAnalyzer:
    ...
    def __init__(self, results_file="../final_dataset/full_100_city_results_20250911_121246.json"):
    ...
    def analyze_pollutant_performance(self, pollutant):
        ...
        for city_name, city_data in self.results.items():
            ...
            if pollutant in city_data['results']:
                ...
                ridge_vs_cams = ((cams_mae - ridge_mae) / cams_mae) * 100 if cams_mae > 0 else 0
                ridge_vs_noaa = ((noaa_mae - ridge_mae) / noaa_mae) * 100 if noaa_mae > 0 else 0
                best_benchmark_mae = min(cams_mae, noaa_mae)
                ridge_vs_best = ((best_benchmark_mae - ridge_mae) / best_benchmark_mae) * 100 if best_benchmark_mae > 0 else 0
        ...
        print(f"Average MAE - Ridge: {df['Ridge_MAE'].mean():.3f}, CAMS: {df['CAMS_MAE'].mean():.3f}, NOAA: {df['NOAA_MAE'].mean():.3f}")
        print(f"Average R² - Ridge: {df['Ridge_R2'].mean():.3f}, CAMS: {df['CAMS_R2'].mean():.3f}, NOAA: {df['NOAA_R2'].mean():.3f}")
        ...
        top_improvements = df.nlargest(10, 'Ridge_vs_Best_%')[['City', 'Continent', 'Ridge_vs_Best_%']]
        ...
        if len(underperformed) > 0:
            print(f"\nCITIES WHERE RIDGE UNDERPERFORMED:")
            print(underperformed[['City', 'Continent', 'Ridge_vs_Best_%']].to_string(index=False))
        else:
                    ...
                    if method in health_data:
                        for alert_type in ['sensitive', 'general']:
                            if alert_type in health_data[method]:
                                ...
                                # Continental totals
                                continental_breakdown[continent][method][alert_type]['tp'] += alert_data.get('true_positives', 0)
                                continental_breakdown[continent][method][alert_type]['fp'] += alert_data.get('false_positives', 0)
                                continental_breakdown[continent][method][alert_type]['tn'] += alert_data.get('true_negatives', 0)
                                continental_breakdown[continent][method][alert_type]['fn'] += alert_data.get('false_negatives', 0)
        
        # Calculate and display metrics
        print(f"\nGLOBAL HEALTH WARNING PERFORMANCE:")
        print(f"{'Method':<8} {'Alert Type':<12} {'Precision':<10} {'Recall':<8} {'FPR':<8} {'FNR':<8} {'F1':<8}")
        print(f"{'-'*70}")
        ...
        for method in self.methods:
            for alert_type in ['sensitive', 'general']:
                ...
                if total > 0:
                    ...
                    print(f"{method.upper():<8} {alert_type:<12} {precision:<10.3f} {recall:<8.3f} {fpr:<8.3f} {fnr:<8.3f} {f1:<8.3f}")
        
        # Continental breakdown for Ridge (best method)
        print(f"\nRIDGE PERFORMANCE BY CONTINENT:")
        print(f"{'Continent':<15} {'Alert Type':<12} {'Precision':<10} {'Recall':<8} {'FPR':<8} {'FNR':<8}")
        print(f"{'-'*70}")
        ...
        for continent in sorted(continental_breakdown.keys()):
            for alert_type in ['sensitive', 'general']:
                ...
                if total > 0:
                    ...
                    print(f"{continent:<15} {alert_type:<12} {precision:<10.3f} {recall:<8.3f} {fpr:<8.3f} {fnr:<8.3f}")
    ...
    def generate_city_rankings(self):
        ...
        print(df.head(20)[['City', 'Country', 'Continent', 'Avg_Improvement_%']].to_string(index=False))
        
        print(f"\nBOTTOM 10 CITIES BY AVERAGE IMPROVEMENT:")
        print(df.tail(10)[['City', 'Country', 'Continent', 'Avg_Improvement_%']].to_string(index=False))
        
        print(f"\nIMPROVEMENT BY CONTINENT:")
        continental_improvement = df.groupby('Continent')['Avg_Improvement_%'].agg(['mean', 'std', 'count']).round(2)
    ...
    def run_complete_analysis(self):
        ...
        print(f"Sensitive Population: {sens_precision:.1%} precision, {sens_fnr:.1%} false negative rate") 
        print(f"General Population: {gen_precision:.1%} precision, {gen_fnr:.1%} false negative rate")
