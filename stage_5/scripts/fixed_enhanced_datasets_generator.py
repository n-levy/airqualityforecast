...
class FixedEnhancedTwoYearDatasetsGenerator:
    ...
    def generate_enhanced_daily_data(self, city_name):
        ...
        all_features = {
            ...
            'Has_Religious_Holidays': bool(city_row.get('Has_Religious_Holidays', True)),
            'Has_National_Holidays': bool(city_row.get('Has_National_Holidays', True)),
            'Holiday_Pollution_Impact': str(city_row.get('Holiday_Pollution_Impact', 'moderate')),
            'Data_Completeness_Score': float(city_row.get('Data_Completeness_Score', 1.0)),
            'Overall_Data_Quality': str(city_row.get('Overall_Data_Quality', 'High')),
            'CAMS_Forecast_Available': bool(city_row.get('CAMS_Forecast_Available', True)),
            'NOAA_Forecast_Available': bool(city_row.get('NOAA_Forecast_Available', True)),
            'Benchmark_Quality_Score': float(city_row.get('Benchmark_Quality_Score', 0.92))
        }
        ...
        for timestamp in timestamps:
            ...
            record = {
                ...
                "forecast_spread": round(float(abs(cams_forecast_aqi - noaa_forecast_aqi)), 1),
    ...
    def generate_and_save_enhanced_datasets(self):
        ...
        safe_print(f"Data Processing: All types converted to JSON-serializable during generation")
        ...
        daily_file = Path("..") / "final_dataset" / f"FIXED_ENHANCED_daily_dataset_{timestamp}.json"
        hourly_file = Path("..") / "final_dataset" / f"FIXED_ENHANCED_hourly_dataset_{timestamp}.json"
        ...
        for idx, city in enumerate(self.cities_df["City"]):
            try:
                safe_print(f"[{idx+1}/100] Processing {city} with JSON-SAFE data generation...")
        ...
        results = {
            ...
            "dataset_comparison": {
                "daily_dataset": {
                    ...
                    "days_per_city": daily_total // len(daily_data) if daily_data else 0
                },
                "hourly_dataset": {
                    ...
                    "components": ["ground_truth", "cams_forecast", "noaa_forecast", "all_features"],
                    "hours_per_city": hourly_total // len(hourly_data) if hourly_data else 0
                },
                "ratios": {
                    "record_ratio": f"{hourly_total / daily_total:.1f}x" if daily_total > 0 else "N/A",
            ...
            "file_locations": {
                ...
                "analysis_results": str(Path("..") / "final_dataset" / f"FIXED_ENHANCED_analysis_{timestamp}.json")
            }
        }

        # Save analysis results
        analysis_file = Path("..") / "final_dataset" / f"FIXED_ENHANCED_analysis_{timestamp}.json"
...
def main():
    ...
    try:
        ...
        daily_file, hourly_file, analysis_file, results = generator.generate_and_save_enhanced_datasets()
