...
class CityReplacementStrategy:
    ...
    def __init__(self):
        ...
        self.current_cities_df = pd.read_csv(
    ...
    def analyze_collection_results(self, collection_file_path):
        ...
        noaa_failed = [
            city
            for city, data in noaa_data.items()
        ...
        waqi_failed = [
            city
            for city, data in waqi_data.items()
            if data.get("api_status") != "SUCCESS"
        ]
        ...
        for city in cities_without_data:
            city_row = self.current_cities_df[
                self.current_cities_df["City"] == city
            ].iloc[0]
    ...
    def find_replacement_cities(self, cities_needing_replacement):
        ...
        for continent, failed_cities in cities_needing_replacement.items():
            ...
            for backup_city in backup_cities:
                ...
                data_availability = self.test_backup_city_data_availability(
                    backup_city, continent
                )
    ...
    def generate_replacement_report(
        self, cities_needing_replacement, recommended_replacements
    ):
        ...
        self.replacement_results.update(
            {
                ...
                "summary": {
                    "total_cities_to_replace": sum(
                        len(cities) for cities in cities_needing_replacement.values()
                    ),
        ...
        for continent, replacements in recommended_replacements.items():
            ...
            available_count = len(
                [
                    r
                    for r in replacements["backup_options"]
                    if r["data_availability"]["data_quality_score"] > 0
                ]
            )

            self.replacement_results["summary"]["replacement_feasibility"][
                continent
            ] = {
                "cities_to_replace": failed_count,
                "viable_replacements": available_count,
                "feasibility_ratio": (
                    available_count / failed_count if failed_count > 0 else 1.0
                ),
            }
        ...
        # Print summary
        total_replacements = self.replacement_results["summary"][
            "total_cities_to_replace"
        ]
        print(f"\nREPLACEMENT ANALYSIS SUMMARY:")
        print(f"Cities requiring replacement: {total_replacements}")

        for continent, feasibility in self.replacement_results["summary"][
            "replacement_feasibility"
        ].items():
            print(
                f"{continent}: {feasibility['cities_to_replace']} to replace, {feasibility['viable_replacements']} viable options"
            )
...
def main():
    ...
    if collection_files:
        ...
        # Analyze results
        cities_needing_replacement = strategy.analyze_collection_results(
            latest_collection
        )

        if cities_needing_replacement:
            # Find replacements
            recommended_replacements = strategy.find_replacement_cities(
                cities_needing_replacement
            )
        ...
        print(
            "No collection results found. Run comprehensive_real_data_collector.py first."
        )
        return {}, None
