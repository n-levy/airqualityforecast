...
def collect_all_real_data():
    ...
    cities_with_real_data.update(
        [
            city
            for city, data in collection_results["noaa_data"].items()
    ...
    cities_with_real_data.update(
        [
            city
            for city, data in collection_results["waqi_data"].items()
            if data.get("api_status") == "SUCCESS"
        ]
    )
    ...
    collection_results["success_counts"]["total_real_data_cities"] = len(
    ...
    print(
        f"  WAQI air quality data: {waqi_success} cities ({waqi_success/total_cities*100:.1f}% success rate)"
    )
