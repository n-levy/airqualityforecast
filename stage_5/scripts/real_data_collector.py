...
class RealDataCollector:
    ...
    def test_api_connections(self) -> Dict[str, Any]:
        ...
        for api_name, config in self.api_configs.items():
            ...
            try:
                ...
                log.info(
                    f"  {config['name']}: {response.status_code} - {'✅' if response.status_code in [200, 401, 403] else '❌'}"
                )
    ...
    def _collect_city_sample_data(self, city: Dict) -> Dict[str, Any]:
        ...
        successful_apis = sum(
            1
            for result in city_result["api_results"].values()
    ...
    def _collect_openaq_data(self, city: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if measurements:
                    ...
                    return {
                        ...
                        "parameters_found": list(
                            set(m.get("parameter") for m in measurements)
                        ),
            ...
            return {"status": "error", "source": "OpenAQ", "error": str(e)}
    ...
    def generate_collection_report(self) -> Dict[str, Any]:
        ...
        if "sample_results" in self.collection_results:
            ...
            total_sample_points = sum(
                result.get("sample_data_points", 0)
                for result in sample_results.values()
            )
            ...
            sample_summary = {
                ...
                "cities_success_rate": (
                    cities_with_data / len(sample_results) if sample_results else 0
                ),
                "total_sample_data_points": total_sample_points,
                "average_points_per_city": (
                    total_sample_points / len(sample_results) if sample_results else 0
                ),
            }
        ...
        overall_assessment = {
            "collection_feasibility": (
                "viable"
                if api_summary.get("accessible_apis", 0) >= 2
                else "challenging"
            ),
            "data_availability": (
                "good" if sample_summary.get("cities_with_data", 0) >= 3 else "limited"
            ),
            "recommended_approach": self._get_collection_recommendations(),
            "estimated_success_rate": min(
                0.8, sample_summary.get("cities_success_rate", 0) + 0.2
            ),
            "next_steps": [
    ...
    def _get_collection_recommendations(self) -> List[str]:
        ...
        if "api_results" in self.collection_results:
            accessible_apis = [
                name
                for name, result in self.collection_results["api_results"].items()
                if result.get("status") == "accessible"
            ]
...
def main():
    ...
    try:
        ...
        log.info(
            f"Feasibility: {report['overall_assessment']['collection_feasibility'].upper()}"
        )
        log.info(
            f"Success Rate: {report['overall_assessment']['estimated_success_rate']:.1%}"
        )
        log.info("=" * 60)
