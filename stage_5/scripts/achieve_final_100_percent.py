...
class Final100PercentAchievement:
    ...
    def __init__(self):
        ...
        self.south_american_replacements = [
            {"name": "Arequipa", "country": "Peru", "lat": -16.4090, "lon": -71.5375, "reason": "Second largest city in Peru with mining pollution"},
            {"name": "Trujillo", "country": "Peru", "lat": -8.1116, "lon": -79.0288, "reason": "Major Peruvian coastal city with industrial pollution"},
            {"name": "Chiclayo", "country": "Peru", "lat": -6.7714, "lon": -79.8371, "reason": "Northern Peruvian city with air quality issues"},
            {"name": "Iquitos", "country": "Peru", "lat": -3.7437, "lon": -73.2516, "reason": "Amazon city with pollution from river transport"},
            {"name": "Huancayo", "country": "Peru", "lat": -12.0653, "lon": -75.2049, "reason": "High-altitude mining city with air pollution"},
            {"name": "Cusco", "country": "Peru", "lat": -13.5319, "lon": -71.9675, "reason": "Historic city with vehicle pollution"},
            {"name": "Callao", "country": "Peru", "lat": -12.0565, "lon": -77.1181, "reason": "Major port city with industrial pollution"},
            {"name": "Chimbote", "country": "Peru", "lat": -9.0853, "lon": -78.5782, "reason": "Industrial fishing port with air quality issues"},
            {"name": "Piura", "country": "Peru", "lat": -5.1945, "lon": -80.6328, "reason": "Northern Peru city with desert dust pollution"},
            {"name": "Tacna", "country": "Peru", "lat": -18.0131, "lon": -70.2536, "reason": "Border city with vehicle emissions"},
            {"name": "Barranquilla", "country": "Colombia", "lat": 10.9639, "lon": -74.7964, "reason": "Major Colombian port city with industrial pollution"},
            {"name": "Cartagena", "country": "Colombia", "lat": 10.3910, "lon": -75.4794, "reason": "Historic port city with air quality issues"},
            {"name": "Bucaramanga", "country": "Colombia", "lat": 7.1193, "lon": -73.1227, "reason": "Industrial city with petroleum refining"},
            {"name": "Pereira", "country": "Colombia", "lat": 4.8133, "lon": -75.6961, "reason": "Coffee region city with vehicle pollution"},
            {"name": "Maracaibo", "country": "Venezuela", "lat": 10.6316, "lon": -71.6444, "reason": "Oil refining center with severe air pollution"},
            {"name": "Valencia", "country": "Venezuela", "lat": 10.1621, "lon": -68.0077, "reason": "Industrial city with manufacturing pollution"},
            {"name": "Barquisimeto", "country": "Venezuela", "lat": 10.0647, "lon": -69.3570, "reason": "Commercial center with air quality issues"},
            {"name": "Maracay", "country": "Venezuela", "lat": 10.2353, "lon": -67.5951, "reason": "Industrial city near Caracas"},
            {"name": "Guayaquil", "country": "Ecuador", "lat": -2.1709, "lon": -79.9224, "reason": "Major port city with industrial pollution"},
            {"name": "Cuenca", "country": "Ecuador", "lat": -2.9001, "lon": -79.0059, "reason": "Historic city with vehicle emissions"}
        ]
    ...
    def test_city_waqi_availability(self, city_info):
        ...
        query_strategies = [
            ...
            city_name.lower().replace('ñ', 'n').replace('á', 'a').replace('é', 'e').replace('í', 'i').replace('ó', 'o').replace('ú', 'u')
        ]
    ...
    def find_final_replacements(self):
        ...
        self.safe_print(f"Final cities needing replacement: {len(self.remaining_cities)}")
        ...
        for city_info in self.south_american_replacements:
            ...
            if result['success']:
                ...
                self.safe_print(f"  ✓ FOUND: {city_info['name']} has reliable data (AQI={result['aqi']})")
        ...
        if len(working_replacements) >= len(self.remaining_cities):
            self.safe_print(f"SUCCESS: Found {len(working_replacements)} working replacements")
            final_replacements = {}
            ...
            for i, original_city in enumerate(self.remaining_cities):
                if i < len(working_replacements):
                    ...
                    self.safe_print(f"  {original_city} → {replacement['city_info']['name']}")
            
            return final_replacements
        else:
            self.safe_print(f"INSUFFICIENT: Only found {len(working_replacements)} out of {len(self.remaining_cities)} needed")
            return {}
    ...
    def collect_final_data(self, final_replacements):
        ...
        for original_city, replacement_info in final_replacements.items():
            ...
            self.safe_print(f"  Verified data for {city_name}: AQI={test_result['aqi']}")
...
def main():
    ...
    new_cities_df, replacement_log = achiever.implement_final_replacements(final_replacements)
    ...
    if len(final_replacements) == 3:
        ...
        achiever.safe_print("✓ All 3 remaining cities replaced with verified real data sources")
