...
class ReplacementCitiesDataCollector:
    ...
    def collect_noaa_data_for_us_cities(self):
        ...
        self.safe_print(
            f"Found {len(us_cities)} US replacement cities for NOAA data collection"
        )
                ...
                if grid_response.status_code == 200:
                    ...
                    if forecast_response.status_code == 200:
                        ...
                        self.collected_data["noaa_data"][city_name] = {
                            ...
                            "hourly_periods": len(
                                hourly_data.get("properties", {}).get("periods", [])
                            ),
                        }
                        ...
                        self.safe_print(
                            f"    SUCCESS: {daily_count} daily + {hourly_count} hourly periods"
                        )
                        ...
                        self.safe_print(
                            f"    FAILED: Forecast unavailable ({forecast_response.status_code})"
                        )
                    ...
                    self.safe_print(
                        f"    FAILED: Grid unavailable ({grid_response.status_code})"
                    )
        ...
        noaa_success = sum(
            1
            for city_data in self.collected_data["noaa_data"].values()
    ...
    def collect_waqi_data_for_replacement_cities(self):
        ...
        for city_name in self.replacement_cities:
            ...
            city_queries = [
                ...
                .replace("í", "i")
                .replace("â", "a"),
                f"{city_name.lower().replace(' ', '-')}-{country.lower()}".replace(
                    "'", ""
                )
                .replace("ã", "a")
                .replace("í", "i")
        ...
        waqi_success = sum(
            1
            for city_data in self.collected_data["waqi_data"].values()
            if city_data.get("api_status") == "SUCCESS"
        )
        self.safe_print(
    ...
    def generate_collection_summary(self):
        ...
        self.collected_data["collection_summary"] = {
            ...
            "real_data_percentage": len(cities_with_real_data)
            / len(self.replacement_cities)
            * 100,
        ...
        self.safe_print(
            f"Cities with real data: {len(cities_with_real_data)} ({len(cities_with_real_data)/len(self.replacement_cities)*100:.1f}%)"
        )
    ...
    def save_results(self):
        ...
        output_file = (
            f"../final_dataset/replacement_cities_data_collection_{timestamp}.json"
        )
