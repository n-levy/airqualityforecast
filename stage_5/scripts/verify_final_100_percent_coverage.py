...
def verify_final_coverage():
    ...
    with open(
        "../final_dataset/replacement_cities_data_collection_20250911_194712.json",
        "r",
        encoding="utf-8",
    ) as f:
    ...
    print(
        f"Total unique cities from replacement collection: {replacement_noaa_count + replacement_waqi_count}"
    )
    ...
    for continent in cities_df["Continent"].unique():
        ...
        continent_real = len(
            [city for city in continent_cities if city in cities_with_real_data]
        )
        ...
        print(
            f"{continent}: {continent_real}/{len(continent_cities)} ({continent_real/len(continent_cities)*100:.1f}%)"
        )
