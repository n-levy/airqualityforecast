...
class Full100CityEvaluator:
    ...
    def generate_efficient_time_series(self, city_row: pd.Series, days: int = 60) -> pd.DataFrame:
        ...
        for i, date in enumerate(dates):
            ...
            seasonal_factor = 1 + 0.15 * np.sin(2 * np.pi * date.timetuple().tm_yday / 365)
            ...
            data.append({
                ...
                'temperature': 20 + 12 * np.sin(2 * np.pi * date.timetuple().tm_yday / 365) + np.random.normal(0, 2),
    ...
    def calculate_local_aqi(self, pm25: float, pm10: float, no2: float, o3: float, 
                           so2: float, co: float, continent: str) -> float:
    ...
    def evaluate_single_city(self, city_data: pd.DataFrame, city_name: str, continent: str) -> Dict:
        ...
        for i in range(max(5, len(city_data)-20), len(city_data)):
            ...
            for pollutant in pollutants:
                ...
                try:
                    if len(train_data) > 3:
                        X_train = train_data[['temperature', 'humidity', 'wind_speed', 
                        ...
                        X_test = np.array([test_data['temperature'], test_data['humidity'], 
                                         test_data['wind_speed'], test_data[f'CAMS_{pollutant}'], 
        ...
        for pollutant in pollutants + ['AQI']:
            ...
            for method in ['simple_avg', 'ridge', 'cams', 'noaa']:
                ...
                r2 = r2_score(actual_values, pred_values) if len(set(actual_values)) > 1 else 0
                mpe = np.mean((pred_values - actual_values) / (actual_values + 1e-8)) * 100
        ...
        health_results = self.analyze_health_warnings(predictions, health_thresholds, continent)
        results['health_warnings'] = health_results
        
        return results
    
    def analyze_health_warnings(self, predictions: Dict, health_thresholds: Dict, continent: str) -> Dict:
    ...
    def process_all_cities(self) -> Dict:
        ...
        for idx, city_row in cities_df.iterrows():
            ...
            try:
                logger.info(f"Processing {city_name}, {continent} ({idx+1}/{total_cities})")
                
                # Generate time series
                city_time_series = self.generate_efficient_time_series(city_row)
                
                # Evaluate city
                city_results = self.evaluate_single_city(city_time_series, city_name, continent)
    ...
    def generate_comprehensive_summary(self, results: Dict) -> Dict:
        ...
        summary = {
            'evaluation_metadata': {
                ...
                'continents_covered': len(set(city['continent'] for city in results.values())),
        ...
        for continent in self.continental_standards.keys():
            continent_cities = {k: v for k, v in results.items() 
        ...
        summary['health_warning_analysis'] = self.analyze_global_health_warnings(results)
        ...
        for pollutant in self.pollutants + ['AQI']:
            ...
            best_ensemble_mae = min([summary['overall_performance'][pollutant][m]['mean_MAE'] 
                                   for m in ensemble_methods])
            best_benchmark_mae = min([summary['overall_performance'][pollutant][m]['mean_MAE'] 
                                    for m in benchmark_methods])
            
            improvement_pct = ((best_benchmark_mae - best_ensemble_mae) / best_benchmark_mae) * 100 if best_benchmark_mae > 0 else 0
            
            summary['improvement_analysis'][pollutant] = {
                'improvement_percent': improvement_pct,
                'best_ensemble_mae': best_ensemble_mae,
                'best_benchmark_mae': best_benchmark_mae,
                'significance': 'major' if improvement_pct > 20 else 'moderate' if improvement_pct > 10 else 'minor'
            }
    ...
    def analyze_global_health_warnings(self, results: Dict) -> Dict:
                    ...
                    if method in health_data:
                        for alert_type in ['sensitive', 'general']:
                            if alert_type in health_data[method]:
                                ...
                                method_metrics[f'{alert_type}_alerts']['tp'] += alert_data.get('true_positives', 0)
                                method_metrics[f'{alert_type}_alerts']['fp'] += alert_data.get('false_positives', 0)
                                method_metrics[f'{alert_type}_alerts']['tn'] += alert_data.get('true_negatives', 0)
                                method_metrics[f'{alert_type}_alerts']['fn'] += alert_data.get('false_negatives', 0)
        ...
        ridge_sensitive = global_health_analysis['overall_metrics'].get('ridge', {}).get('sensitive_alerts', {})
        ridge_general = global_health_analysis['overall_metrics'].get('ridge', {}).get('general_alerts', {})
        ...
        global_health_analysis['false_positive_analysis'] = {
            ...
            'impact_assessment': 'Acceptable' if ridge_sensitive.get('false_positive_rate', 0) < 0.15 else 'Needs Optimization'
        }
        ...
        global_health_analysis['false_negative_analysis'] = {
            ...
            'impact_assessment': 'Acceptable' if ridge_sensitive.get('false_negative_rate', 0) < 0.10 else 'Critical Issue'
        }
...
def main():
    ...
    for pollutant, improvement in summary['improvement_analysis'].items():
        print(f"  {pollutant}: {improvement['improvement_percent']:.1f}% ({improvement['significance'].upper()})")
    
    # Health warning analysis
    health_analysis = summary['health_warning_analysis']
    
    print(f"\nHealth Warning Analysis:")
    ridge_sensitive = health_analysis['overall_metrics'].get('ridge', {}).get('sensitive_alerts', {})
    ridge_general = health_analysis['overall_metrics'].get('ridge', {}).get('general_alerts', {})
    ...
    print(f"    False Positive Rate: {ridge_sensitive.get('false_positive_rate', 0):.1%}")
    print(f"    False Negative Rate: {ridge_sensitive.get('false_negative_rate', 0):.1%}")
