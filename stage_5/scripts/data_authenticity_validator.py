...
class DataAuthenticityValidator:
    ...
    def validate_dataset_sample(self, dataset_file, sample_size=5):
        ...
        try:
            ...
            if "cities_data" in dataset:
                ...
                cities_data = [
                    {
                        "city_metadata": {"name": city, "country": "Unknown"},
                        "daily_records": (
                            records[:10] if isinstance(records, list) else [records]
                        ),
                    }
            ...
            sample_cities = random.sample(
                cities_data, min(sample_size, len(cities_data))
            )
            ...
            for i, city_data in enumerate(sample_cities):
                ...
                print(
                    f"  [{i+1}/{len(sample_cities)}] Validating {city_name}, {country}..."
                )
                ...
                if current_api_data and "error" not in current_api_data:
                    ...
                    validation = {
                        ...
                        "city_name_match": city_name.lower()
                        in current_api_data.get("api_city", "").lower(),
                        "has_coordinates": len(
                            current_api_data.get("api_coordinates", [])
                        )
                        == 2,
                        "has_pollutants": len(
                            current_api_data.get("api_pollutants", {})
                        )
                        > 0,
                        "data_freshness": (
                            "recent"
                            if current_api_data.get("api_timestamp")
                            else "unknown"
                        ),
                    ...
                    if validation["api_accessible"]:
                        score += 30
                    if validation["city_name_match"]:
                        score += 25
                    if validation["has_coordinates"]:
                        score += 20
                    if validation["has_pollutants"]:
                        score += 15
                    if validation["api_aqi"] and validation["baseline_aqi"]:
                        aqi_diff = abs(
                            validation["api_aqi"] - validation["baseline_aqi"]
                        )
                        if aqi_diff <= 50:
                    ...
                    print(
                        f"    ✅ API Accessible: AQI={validation['api_aqi']}, Score={score}/100"
                    )
                    ...
                    print(
                        f"    ❌ API Failed: {validation.get('error', 'Unknown error')}"
                    )
            ...
            accessible_count = sum(
                1 for v in validation_results if v.get("api_accessible", False)
            )
            high_authenticity = sum(
                1 for v in validation_results if v.get("authenticity_level") == "HIGH"
            )
            ...
            summary = {
                ...
                "overall_authenticity": (
                    "100% REAL"
                    if accessible_count == len(sample_cities)
                    and high_authenticity >= len(sample_cities) * 0.8
                    else (
                        "MOSTLY REAL"
                        if accessible_count >= len(sample_cities) * 0.7
                        else "QUESTIONABLE"
                    )
                ),
    ...
    def save_validation_results(self, validation_summary):
        ...
        results_file = (
            f"../final_dataset/data_authenticity_validation_{timestamp_str}.json"
        )
...
def main():
    ...
    for dataset_file in dataset_files[:3]:  # Validate first 3 datasets
        if "sample" not in dataset_file.lower():  # Skip sample files
            validation_result = validator.validate_dataset_sample(
                dataset_file, sample_size=3
            )
