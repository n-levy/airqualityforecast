...
def load_results(results_file="../final_dataset/full_100_city_results_20250911_121246.json"):
...
def analyze_pollutant_comparison(results, pollutant):
    ...
    for city_name, city_info in results.items():
        ...
        if pollutant in city_info['results']:
            ...
            ridge_improvement = ((best_benchmark_mae - ridge_mae) / best_benchmark_mae) * 100 if best_benchmark_mae > 0 else 0
    ...
    report_lines.append(f"Average MAE - Ridge: {df['Ridge_MAE'].mean():.3f}, CAMS: {df['CAMS_MAE'].mean():.3f}, NOAA: {df['NOAA_MAE'].mean():.3f}")
    report_lines.append(f"Average R2 - Ridge: {df['Ridge_R2'].mean():.3f}, CAMS: {df['CAMS_R2'].mean():.3f}, NOAA: {df['NOAA_R2'].mean():.3f}")
    report_lines.append(f"Ridge Improvement over Best Benchmark: {df['Ridge_Improvement_%'].mean():.1f}% average")
    ...
    for continent in continental_stats.index:
        ...
        report_lines.append(f"{continent}: Ridge {stats['Ridge_MAE']:.3f}, CAMS {stats['CAMS_MAE']:.3f}, NOAA {stats['NOAA_MAE']:.3f}, Improvement {stats['Ridge_Improvement_%']:.1f}%")
    ...
    for i, (_, row) in enumerate(df_sorted.head(10).iterrows()):
        report_lines.append(f"{i+1:2d}. {row['City']:<15} ({row['Continent']:<15}): {row['Ridge_Improvement_%']:6.1f}%")
    ...
    if len(underperformed) > 0:
        ...
        for _, row in underperformed.iterrows():
            report_lines.append(f"  {row['City']:<15} ({row['Continent']:<15}): {row['Ridge_Improvement_%']:6.1f}%")
    else:
...
def analyze_aqi_health_warnings(results):
                ...
                if method in health_data:
                    for alert_type in ['sensitive', 'general']:
                        if alert_type in health_data[method]:
                            ...
                            # Global totals
                            method_totals[method][alert_type]['tp'] += alert_data.get('true_positives', 0)
                            method_totals[method][alert_type]['fp'] += alert_data.get('false_positives', 0)
                            method_totals[method][alert_type]['tn'] += alert_data.get('true_negatives', 0)
                            method_totals[method][alert_type]['fn'] += alert_data.get('false_negatives', 0)
                            
                            # Continental totals
                            continental_totals[continent][method][alert_type]['tp'] += alert_data.get('true_positives', 0)
                            continental_totals[continent][method][alert_type]['fp'] += alert_data.get('false_positives', 0)
                            continental_totals[continent][method][alert_type]['tn'] += alert_data.get('true_negatives', 0)
                            continental_totals[continent][method][alert_type]['fn'] += alert_data.get('false_negatives', 0)
    
    # Calculate and report metrics
    report_lines.append(f"\nGLOBAL HEALTH WARNING PERFORMANCE:")
    report_lines.append(f"{'Method':<8} {'Alert':<10} {'Precision':<10} {'Recall':<8} {'FPR':<8} {'FNR':<8} {'F1':<8}")
    ...
    for method in methods:
        for alert_type in ['sensitive', 'general']:
            ...
            report_lines.append(f"{method.upper():<8} {alert_type:<10} {precision:<10.3f} {recall:<8.3f} {fpr:<8.3f} {fnr:<8.3f} {f1:<8.3f}")
    
    # Ridge performance by continent
    report_lines.append(f"\nRIDGE HEALTH WARNING PERFORMANCE BY CONTINENT:")
    report_lines.append(f"{'Continent':<15} {'Alert':<10} {'Precision':<10} {'Recall':<8} {'FPR':<8} {'FNR':<8}")
    ...
    for continent in sorted(continental_totals.keys()):
        for alert_type in ['sensitive', 'general']:
            ...
            report_lines.append(f"{continent:<15} {alert_type:<10} {precision:<10.3f} {recall:<8.3f} {fpr:<8.3f} {fnr:<8.3f}")
...
def generate_city_rankings(results):
    ...
    for city_name, city_data in results.items():
        ...
        for pollutant in pollutants:
            if pollutant in city_data['results']:
                ...
                best_benchmark_mae = min(cams_mae, noaa_mae)
                improvement = ((best_benchmark_mae - ridge_mae) / best_benchmark_mae) * 100 if best_benchmark_mae > 0 else 0
                
                total_improvement += improvement
                valid_pollutants += 1
        
        avg_improvement = total_improvement / valid_pollutants if valid_pollutants > 0 else 0
    ...
    for i, (_, row) in enumerate(df.head(20).iterrows()):
        report_lines.append(f"{i+1:2d}. {row['City']:<15} ({row['Country']:<15}, {row['Continent']:<15}): {row['Avg_Improvement_%']:6.1f}%")
    ...
    continental_avg = df.groupby('Continent')['Avg_Improvement_%'].mean().sort_values(ascending=False)
...
def main():
    ...
    for pollutant in pollutants:
        ...
        pollutant_report, pollutant_df = analyze_pollutant_comparison(results, pollutant)
    ...
    for pollutant in pollutants:
        if pollutant in pollutant_dfs:
            ...
            report_content.append(f"{pollutant:<8}: {avg_improvement:6.1f}% average improvement over best benchmark")
    ...
    sens_precision = ridge_sensitive['tp'] / (ridge_sensitive['tp'] + ridge_sensitive['fp']) if (ridge_sensitive['tp'] + ridge_sensitive['fp']) > 0 else 0
    sens_fnr = ridge_sensitive['fn'] / (ridge_sensitive['tp'] + ridge_sensitive['fn']) if (ridge_sensitive['tp'] + ridge_sensitive['fn']) > 0 else 0
    
    gen_precision = ridge_general['tp'] / (ridge_general['tp'] + ridge_general['fp']) if (ridge_general['tp'] + ridge_general['fp']) > 0 else 0
    gen_fnr = ridge_general['fn'] / (ridge_general['tp'] + ridge_general['fn']) if (ridge_general['tp'] + ridge_general['fn']) > 0 else 0
    
    report_content.append(f"\nRIDGE HEALTH WARNING PERFORMANCE:")
    report_content.append(f"Sensitive Population: {sens_precision:.1%} precision, {sens_fnr:.1%} false negative rate")
    report_content.append(f"General Population: {gen_precision:.1%} precision, {gen_fnr:.1%} false negative rate")
    
    # Cities analysis summary
    total_cities = len(results)
    cities_improved = sum(1 for df in pollutant_dfs.values() for _, row in df.iterrows() if row['Ridge_Improvement_%'] > 0) / len(pollutant_dfs)
    
    report_content.append(f"\nCITY PERFORMANCE SUMMARY:")
    report_content.append(f"Total cities evaluated: {total_cities}")
    report_content.append(f"Average cities where Ridge outperformed benchmarks: {cities_improved:.0f} cities per pollutant")
    report_content.append(f"Best performing city overall: {rankings_df.iloc[0]['City']} ({rankings_df.iloc[0]['Avg_Improvement_%']:.1f}% improvement)")
