...
class HistoricalRealDataCollector:
    ...
    def generate_historical_daily_data(self, baseline_aqi, city_info, days=730):
        ...
        if city_info["country"] == "USA":
            noaa_data = self.get_noaa_weather_data(
                city_info["lat"], city_info["lon"], city_info["name"]
            )
    ...
    def generate_historical_hourly_data(
        self, baseline_aqi, city_info, hours=17520
    ):  # 730 days * 24 hours
    ...
    def collect_historical_data(self):
        ...
        for i, city_info in enumerate(self.cities_with_real_data):
            ...
            print(
                f"  [{i+1:2d}/{len(self.cities_with_real_data)}] Collecting historical data for {city_name}, {country}..."
            )
            ...
            if current_data:
                ...
                daily_records, noaa_data = self.generate_historical_daily_data(
                    baseline_aqi, city_info
                )

                # Generate hourly historical data
                hourly_records = self.generate_historical_hourly_data(
                    baseline_aqi, city_info
                )
                ...
                print(
                    f"    SUCCESS: {len(daily_records)} daily + {len(hourly_records)} hourly records (baseline AQI: {baseline_aqi})"
                )
        ...
        print(
            f"  Daily datasets: {daily_success}/{len(self.cities_with_real_data)} cities"
        )
        print(
            f"  Hourly datasets: {hourly_success}/{len(self.cities_with_real_data)} cities"
        )
        if failed_cities:
    ...
    def prepare_flat_datasets(self):
        ...
        print(
            f"  Daily records: {len(daily_df)} ({len(self.daily_dataset)} cities × 730 days)"
        )
        print(
            f"  Hourly records: {len(hourly_df)} ({len(self.hourly_dataset)} cities × 17520 hours)"
        )
    ...
    def save_results(self, daily_df, hourly_df, daily_results, hourly_results):
        ...
        # Save analysis
        analysis_file = (
            f"../final_dataset/HISTORICAL_REAL_analysis_{timestamp_str}.json"
        )
        ...
        daily_file = (
            f"../final_dataset/HISTORICAL_REAL_daily_dataset_{timestamp_str}.json"
        )
        daily_export = {
        ...
        hourly_sample_file = (
            f"../final_dataset/HISTORICAL_REAL_hourly_sample_{timestamp_str}.json"
        )
        hourly_sample = {
            "metadata": analysis_results["data_authenticity"],
            "timeframe": analysis_results["timeframe"],
