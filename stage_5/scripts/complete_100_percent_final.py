...
def complete_final_100_percent():
    ...
    for continent in cities_df['Continent'].unique():
        ...
        continent_real = len(continent_cities[continent_cities['Has_Real_Data'] == True])
        ...
        print(f"{continent}: {continent_real}/{total_continent} ({continent_real/total_continent*100:.1f}%)")
    
    # Save final achievement report
    report_file = f"../final_dataset/final_100_percent_achievement_{timestamp}.json"
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(final_achievement_report, f, indent=2, default=str, ensure_ascii=False)
