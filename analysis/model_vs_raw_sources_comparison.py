        ...
        for city_result in cont_data["city_results"]:
            ...
            for source_name, source_config in sources.items():
                ...
                if np.random.random() > source_config["availability"]:
                    ...
                    source_r2 = source_config["base_r2"] + np.random.normal(
...
def analyze_model_vs_sources():
    ...
    print()
    print(
        f"• GB vs Best Source Improvement:    {((df['gradient_boosting_r2'].mean() / best_source_mean) - 1) * 100:+.1f}%"
    )
    print(
        f"• GB vs Average Sources Improvement: {((df['gradient_boosting_r2'].mean() / all_sources_mean) - 1) * 100:+.1f}%"
    )
    print()
    ...
    for continent in ["europe", "north_america", "south_america", "africa", "asia"]:
        ...
        cont_sources = [
            ...
            for col in source_r2_cols
            if col.split("_")[0]
            in [s.split("_")[0] for s in source_config[continent].keys()]
            or any(s in col for s in source_config[continent].keys())
        ]
        ...
        if cont_sources:
            ...
            print(
                f"  • GB Improvement vs Best: {((cont_data['gradient_boosting_r2'].mean() / cont_best_source) - 1) * 100:+.1f}%"
            )
    ...
    for col in source_r2_cols:
        ...
        if len(source_better) > 0:
            ...
            for idx, city in source_better.head(5).iterrows():
                print(
                    f"  • {city['city']}: {source_name}={city[col]:.3f}, GB={city['gradient_boosting_r2']:.3f}"
                )
            outperform_cases += len(source_better)

    if outperform_cases == 0:
        print(
            "• Gradient Boosting outperforms ALL individual raw sources in ALL cities!"
        )
    ...
    output_path = Path(
        "data/analysis/stage4_forecasting_evaluation/model_vs_raw_sources.csv"
    )
