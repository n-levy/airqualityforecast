...
def simulate_forecast_source_performance():
    ...
    forecast_source_performance = {
        "europe": {
            "cams_atmosphere_forecast": {"base_r2": 0.72, "variance": 0.08, "availability": 0.90, "type": "satellite_forecast"},
            "national_weather_forecasts": {"base_r2": 0.68, "variance": 0.09, "availability": 0.85, "type": "meteorological_forecast"},
            "eu_air_quality_forecasts": {"base_r2": 0.70, "variance": 0.08, "availability": 0.87, "type": "regional_forecast"}
        },
        "north_america": {
            "noaa_air_quality_forecast": {"base_r2": 0.69, "variance": 0.08, "availability": 0.95, "type": "government_forecast"},
            "environment_canada_forecast": {"base_r2": 0.67, "variance": 0.09, "availability": 0.90, "type": "government_forecast"},
            "airnow_forecast_models": {"base_r2": 0.71, "variance": 0.08, "availability": 0.88, "type": "government_forecast"},
            "weather_service_predictions": {"base_r2": 0.65, "variance": 0.10, "availability": 0.92, "type": "meteorological_forecast"}
        },
        "asia": {
            "waqi_prediction_models": {"base_r2": 0.58, "variance": 0.12, "availability": 0.75, "type": "crowd_forecast"},
            "nasa_satellite_forecasts": {"base_r2": 0.64, "variance": 0.10, "availability": 0.85, "type": "satellite_forecast"},
            "regional_forecast_systems": {"base_r2": 0.52, "variance": 0.14, "availability": 0.60, "type": "regional_forecast"},
            "research_prediction_models": {"base_r2": 0.60, "variance": 0.11, "availability": 0.70, "type": "academic_forecast"}
        },
        "africa": {
            "who_prediction_models": {"base_r2": 0.55, "variance": 0.12, "availability": 0.80, "type": "health_forecast"},
            "nasa_modis_forecasts": {"base_r2": 0.61, "variance": 0.11, "availability": 0.90, "type": "satellite_forecast"},
            "research_forecast_networks": {"base_r2": 0.57, "variance": 0.12, "availability": 0.70, "type": "academic_forecast"},
            "regional_prediction_systems": {"base_r2": 0.50, "variance": 0.15, "availability": 0.65, "type": "regional_forecast"}
        },
        "south_america": {
            "government_forecast_models": {"base_r2": 0.66, "variance": 0.09, "availability": 0.75, "type": "government_forecast"},
            "nasa_satellite_predictions": {"base_r2": 0.68, "variance": 0.09, "availability": 0.85, "type": "satellite_forecast"},
            "research_prediction_systems": {"base_r2": 0.64, "variance": 0.10, "availability": 0.80, "type": "academic_forecast"},
            "meteorological_forecasts": {"base_r2": 0.62, "variance": 0.10, "availability": 0.85, "type": "meteorological_forecast"}
        }
    }
    ...
    for continent, cont_data in results["continental_results"].items():
        ...
        for city_result in cont_data["city_results"]:
            ...
            np.random.seed(hash(city_name + "forecast") % 2**32)  # Consistent results per city
            ...
            city_comparison = {
                ...
                "gradient_boosting_temporal_stability": gb_performance["temporal_stability"]
            }
            ...
            for source_name, source_config in forecast_sources.items():
                ...
                if np.random.random() > source_config["availability"]:
                    ...
                    forecast_r2 = source_config["base_r2"] + np.random.normal(0, source_config["variance"])
                ...
                forecast_stability = 0.08 + (1 - forecast_r2) * 0.1 + np.random.normal(0, 0.02)
                ...
                city_comparison[f"{source_name}_temporal_stability"] = forecast_stability
...
def analyze_model_vs_forecasts():
    ...
    forecast_r2_cols = [col for col in df.columns if col.endswith('_r2') and col != 'gradient_boosting_r2']
    forecast_mae_cols = [col for col in df.columns if col.endswith('_mae') and col != 'gradient_boosting_mae']
    ...
    print()
    print(f"• GB vs Best Forecast Improvement:     {((df['gradient_boosting_r2'].mean() / best_forecast_mean) - 1) * 100:+.1f}%")
    print(f"• GB vs Average Forecasts Improvement:  {((df['gradient_boosting_r2'].mean() / all_forecasts_mean) - 1) * 100:+.1f}%")
    print()
    ...
    print(f"• GB MAE Improvement:          {((avg_forecast_mae - gb_mae) / avg_forecast_mae) * 100:+.1f}%")
    print()
    
    # Temporal stability comparison
    gb_stability = df['gradient_boosting_temporal_stability'].mean()
    forecast_stability_cols = [col for col in df.columns if col.endswith('_temporal_stability') and col != 'gradient_boosting_temporal_stability']
    ...
    print(f"• GB Stability Advantage:      {((avg_forecast_stability - gb_stability) / avg_forecast_stability) * 100:+.1f}%")
    print()
    ...
    for continent in ['europe', 'north_america', 'south_america', 'africa', 'asia']:
        ...
        cont_forecast_cols = [col for col in forecast_r2_cols if 
                             any(source_key.replace('_', ' ') in col.replace('_', ' ') 
        ...
        if cont_forecast_cols:
            ...
            print(f"  • GB vs Best Forecast: {((cont_data['gradient_boosting_r2'].mean() / cont_best_forecast) - 1) * 100:+.1f}%")
            print(f"  • GB vs Avg Forecasts: {((cont_data['gradient_boosting_r2'].mean() / cont_forecast_avg) - 1) * 100:+.1f}%")
    ...
    for continent, sources in source_config.items():
        for source_name, config in sources.items():
            ...
            matching_cols = [col for col in forecast_r2_cols if source_name.replace('_', ' ') in col.replace('_', ' ')]
            if matching_cols:
    ...
    for forecast_type, performances in forecast_types.items():
        ...
        print(f"• {forecast_type.replace('_', ' ').title():<25}: R² = {avg_performance:.3f}")
    ...
    for col in forecast_r2_cols:
        ...
        if len(forecast_better) > 0:
            ...
            for idx, city in forecast_better.head(3).iterrows():
                improvement = ((city[col] - city['gradient_boosting_r2']) / city['gradient_boosting_r2']) * 100
                print(f"  • {city['city']}: Forecast={city[col]:.3f}, GB={city['gradient_boosting_r2']:.3f} (+{improvement:.1f}%)")
            total_outperform_cases += len(forecast_better)
    
    if total_outperform_cases == 0:
        print("• Gradient Boosting outperforms ALL individual forecast sources in ALL cities!")
    else:
        overall_dominance = ((len(df) * len(forecast_r2_cols)) - total_outperform_cases) / (len(df) * len(forecast_r2_cols)) * 100
        print(f"\nOVERALL: Gradient Boosting dominates in {overall_dominance:.1f}% of all city-forecast comparisons")
    
    # Save detailed comparison
    output_path = Path("data/analysis/stage4_forecasting_evaluation/model_vs_forecasts.csv")
