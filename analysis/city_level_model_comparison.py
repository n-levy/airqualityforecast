...
def analyze_city_level_performance():
    ...
    for continent, cont_data in results["continental_results"].items():
        for city_result in cont_data["city_results"]:
            ...
            weighted_perf = city_result["model_performance"][
                "quality_weighted_ensemble"
            ]

            # Calculate improvements
            gb_vs_simple_improvement = (
                (gb_perf["r2_score"] - simple_perf["r2_score"])
                / simple_perf["r2_score"]
                * 100
            )
            gb_vs_weighted_improvement = (
                (gb_perf["r2_score"] - weighted_perf["r2_score"])
                / weighted_perf["r2_score"]
                * 100
            )
    ...
    print(
        f"• GB vs Simple Average - Mean Improvement: {df['gb_vs_simple_improvement_pct'].mean():.1f}%"
    )
    print(
        f"• GB vs Quality-Weighted - Mean Improvement: {df['gb_vs_weighted_improvement_pct'].mean():.1f}%"
    )
    print(
        f"• Cities where GB > Simple Average: {(df['gb_vs_simple_improvement_pct'] > 0).sum()}/100"
    )
    print(
        f"• Cities where GB > Quality-Weighted: {(df['gb_vs_weighted_improvement_pct'] > 0).sum()}/100"
    )
    print()
    ...
    print(
        f"• Simple Average Ensemble:    {df['production_ready_simple'].sum()}/100 cities"
    )
    print(
        f"• Quality-Weighted Ensemble:  {df['production_ready_weighted'].sum()}/100 cities"
    )
    print()
    ...
    for continent in ["europe", "north_america", "south_america", "africa", "asia"]:
        ...
        print(
            f"  • GB Production Ready: {cont_data['production_ready_gb'].sum()}/{len(cont_data)} cities"
        )
        print(
            f"  • Mean GB Improvement: {cont_data['gb_vs_weighted_improvement_pct'].mean():.1f}% vs Quality-Weighted"
        )

    # Best and worst performing cities
    print("\nTOP 10 BEST PERFORMING CITIES (Gradient Boosting):")
    top_cities = df.nlargest(10, "gradient_boosting_r2")[
        ["city", "continent", "gradient_boosting_r2"]
    ]
    for idx, city in top_cities.iterrows():
        print(
            f"  {city['city']:<25} ({city['continent']:<15}): R² = {city['gradient_boosting_r2']:.3f}"
        )

    print("\nTOP 10 WORST PERFORMING CITIES (Gradient Boosting):")
    worst_cities = df.nsmallest(10, "gradient_boosting_r2")[
        ["city", "continent", "gradient_boosting_r2"]
    ]
    ...
    if len(underperform_simple) > 0:
        ...
        for idx, city in underperform_simple.iterrows():
            print(
                f"  {city['city']}: GB={city['gradient_boosting_r2']:.3f}, Simple={city['simple_average_r2']:.3f}"
            )
    else:
    ...
    if len(underperform_weighted) > 0:
        ...
        for idx, city in underperform_weighted.iterrows():
            print(
                f"  {city['city']}: GB={city['gradient_boosting_r2']:.3f}, Weighted={city['quality_weighted_r2']:.3f}"
            )
    else:
    ...
    output_path = Path(
        "data/analysis/stage4_forecasting_evaluation/city_level_comparison.csv"
    )
