...
class ComprehensiveHealthValidator:
    ...
    def __init__(
        self, output_dir: str = "data/analysis/stage4_comprehensive_validation"
    ):
    ...
    def generate_synthetic_city_data(
        ...
        for pollutant in self.pollutants:
            data[f"{pollutant}_primary"] = data[pollutant] + np.random.normal(
                0, data[pollutant] * 0.1, total_days
            )
    ...
    def calculate_aqi(self, data: Dict, continent: str) -> np.ndarray:
        ...
        if continent == "europe":
            ...
            no2_aqi = np.where(
                no2 <= 100, no2 * 100 / 100, 100 + (no2 - 100) * 100 / 100
            )
            ...
            o3_aqi = np.where(
                ...
                o3 * 50 / 70,
                np.where(
                    o3 <= 100, 50 + (o3 - 70) * 50 / 30, 100 + (o3 - 100) * 100 / 50
                ),
            )
        ...
        if model_name == "simple_average_ensemble":
            ...
            if pollutant == "aqi":
                ...
                pollutant_indices = [
                    ...
                    -2,
                    -1,
                ]  # Last 5 features are pollutants
                y_pred = (
                    np.mean(X_test[:, pollutant_indices], axis=1) * 15
                )  # Scale to AQI range
            else:
                ...
                y_pred = X_test[
                    :, -5 + pollutant_idx
                ]  # Corresponding primary pollutant
    ...
    def evaluate_health_warnings(
        self, predictions: np.ndarray, actuals: np.ndarray, thresholds: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    def comprehensive_city_evaluation(
        ...
        for target in evaluation_targets:
            ...
            for model_name in self.models.keys():
                ...
                if target == "aqi":
                    health_thresholds = self.continental_patterns[continent][
                        "health_thresholds"
                    ]
    ...
    def evaluate_continental_performance(self, continent: str) -> Dict[str, Any]:
        ...
        for city_idx in range(num_cities):
            ...
            city_results = self.comprehensive_city_evaluation(
    ...
    def aggregate_continental_results(
        ...
        for target in evaluation_targets:
            ...
            for model_name in self.models.keys():
                ...
                for city_result in city_results:
                    if (
                        target in city_result["pollutant_performance"]
                        and model_name in city_result["pollutant_performance"][target]
                    ):
                        city_performance = city_result["pollutant_performance"][target][
                            model_name
                        ]
                ...
                for metric in metrics:
                    if model_metrics[metric]:
                        pollutant_aggregates[target][model_name][f"mean_{metric}"] = (
                            np.mean(model_metrics[metric])
                        )
                        pollutant_aggregates[target][model_name][f"std_{metric}"] = (
                            np.std(model_metrics[metric])
                        )
        ...
        for model_name in self.models.keys():
            ...
            for alert_type in ["sensitive", "general"]:
                ...
                for city_result in city_results:
                    if (
                        model_name in city_result["health_warnings"]
                        and alert_type in city_result["health_warnings"][model_name]
                    ):
                        alert_data = city_result["health_warnings"][model_name][
                            alert_type
                        ]
                ...
                for metric in alert_metrics:
                    if aggregated_metrics[metric]:
                        health_warning_aggregates[model_name][alert_type][
                            f"mean_{metric}"
                        ] = np.mean(aggregated_metrics[metric])
                        health_warning_aggregates[model_name][alert_type][
                            f"std_{metric}"
                        ] = np.std(aggregated_metrics[metric])
        ...
        best_aqi_model = max(
            self.models.keys(),
            key=lambda m: pollutant_aggregates["aqi"]
            .get(m, {})
            .get("mean_r2_score", 0),
        )
    ...
    def evaluate_global_system(self) -> Dict[str, Any]:
        ...
        for continent in self.continental_patterns.keys():
            ...
            best_r2 = continental_results["pollutant_performance"]["aqi"][best_model][
                "mean_r2_score"
            ]

            log.info(
                f"{continent.title()} - Best AQI Model: {best_model} (R² = {best_r2:.3f})"
            )
    ...
    def create_global_summary(
        ...
        for model_name in self.models.keys():
            ...
            for continent_data in continental_results.values():
                # AQI performance
                if (
                    "aqi" in continent_data["pollutant_performance"]
                    and model_name in continent_data["pollutant_performance"]["aqi"]
                ):
                    aqi_performance = continent_data["pollutant_performance"]["aqi"][
                        model_name
                    ]
                ...
                if model_name in continent_data["health_warning_performance"]:
                    health_data = continent_data["health_warning_performance"][
                        model_name
                    ]

                    if "sensitive" in health_data:
                        total_fn_sensitive += health_data["sensitive"].get(
                            "overall_false_positive_count", 0
                        )
                        total_alerts_sensitive += health_data["sensitive"].get(
                            "total_health_events", 0
                        )

                    if "general" in health_data:
                        total_fn_general += health_data["general"].get(
                            "overall_false_positive_count", 0
                        )
                        total_alerts_general += health_data["general"].get(
                            "total_health_events", 0
                        )
            ...
            global_health_performance[model_name] = {
                "sensitive_false_negative_rate": (
                    total_fn_sensitive / total_alerts_sensitive
                    if total_alerts_sensitive > 0
                    else 0
                ),
                "general_false_negative_rate": (
                    total_fn_general / total_alerts_general
                    if total_alerts_general > 0
                    else 0
                ),
        ...
        best_model_fn_rate = global_health_performance[best_health_model][
            "sensitive_false_negative_rate"
        ]
        ...
        return {
            ...
            "continental_ranking": sorted(
                [
                    (
                        cont,
                        data["pollutant_performance"]["aqi"][data["best_aqi_model"]][
                            "mean_r2_score"
                        ],
                    )
            ...
            "global_readiness": {
                "aqi_prediction_ready": global_aqi_performance[best_aqi_model][
                    "global_mean_aqi_r2"
                ]
                > 0.75,
                "health_warnings_ready": health_criteria_met,
                "production_ready": (
                    global_aqi_performance[best_aqi_model]["global_mean_aqi_r2"] > 0.75
                )
                and health_criteria_met,
            },
        }
            ...
            health_summary.append(
                {
                    ...
                    "model_type": self.models[model_name]["type"],
                    "sensitive_false_negative_rate": performance[
                        "sensitive_false_negative_rate"
                    ],
                    "general_false_negative_rate": performance[
                        "general_false_negative_rate"
                    ],
                    "sensitive_health_events": performance["sensitive_health_events"],
                    "general_health_events": performance["general_health_events"],
                    "health_criteria_met": performance["sensitive_false_negative_rate"]
                    < 0.10,
                }
            )
...
def main():
    ...
    print(
        f"• Health Criteria Met: {'✅' if global_summary['health_criteria_met'] else '❌'}"
    )
    ...
    print(
        f"• AQI Prediction Ready: {'✅' if readiness['aqi_prediction_ready'] else '❌'}"
    )
    print(
        f"• Health Warnings Ready: {'✅' if readiness['health_warnings_ready'] else '❌'}"
    )
    ...
    if readiness["production_ready"]:
        ...
        print(
            "Individual pollutants + AQI + Health warnings validated across 100 cities"
        )
    else:
