...
class QuickWalkForwardDemo:
    ...
    def load_sample_data(self):
        ...
        logger.info(
            f"Date range: {sampled_data['datetime'].min()} to {sampled_data['datetime'].max()}"
        )
    ...
    def run_demo_validation(self):
        ...
        for idx, row in test_data.iterrows():
            result_row = {"datetime": row["datetime"], "city": row["city"]}
            ...
            for pollutant in self.pollutants:
                ...
                result_row[f"noaa_{pollutant}"] = row[
                    f"forecast_noaa_gefs_aerosol_{pollutant}"
                ]

                # Simple average
                simple_avg = (
                    row[f"forecast_cams_{pollutant}"]
                    + row[f"forecast_noaa_gefs_aerosol_{pollutant}"]
                ) / 2
                ...
                X_test = np.array(
                    [row[col] if col in row else 0 for col in feature_cols]
                ).reshape(1, -1)
            ...
            current_train = pd.concat(
                [current_train, pd.DataFrame([row])], ignore_index=True
            )
    ...
    def evaluate_and_report(self, results_df):
        ...
        report_lines.extend(["", "## ENSEMBLE IMPROVEMENTS:", ""])
