...
class ComprehensiveAnalyzer:
    ...
    def analyze_model_performance(self, metrics_df):
            ...
            performance_summary.append(
                {
                    ...
                    "avg_r2": avg_r2,
                    "model_type": (
                        "benchmark" if model in ["cams", "noaa"] else "ensemble"
                    ),
                }
            )
        ...
        performance_df["cams_improvement_pct"] = (
            (cams_mae - performance_df["avg_mae"]) / cams_mae
        ) * 100
        performance_df["noaa_improvement_pct"] = (
            (noaa_mae - performance_df["avg_mae"]) / noaa_mae
        ) * 100
    ...
    def analyze_pollutant_performance(self, metrics_df):
        ...
        for pollutant in self.pollutants:
            ...
            cams_mae = pollutant_metrics[pollutant_metrics["model"] == "cams"][
                "mae"
            ].iloc[0]
            noaa_mae = pollutant_metrics[pollutant_metrics["model"] == "noaa"][
                "mae"
            ].iloc[0]

            # Calculate improvements for ensemble models
            ensemble_models = pollutant_metrics[
                pollutant_metrics["model"].isin(
                    ["simple_average", "ridge", "gradient_boosting"]
                )
            ]
    ...
    def generate_comprehensive_report(self, performance_df, pollutant_df, metrics_df):
        ...
        ensemble_performance = performance_df[
            performance_df["model_type"] == "ensemble"
        ].sort_values("avg_mae")
        ...
        report_lines.extend(["## BENCHMARK PERFORMANCE", ""])

        benchmark_performance = performance_df[
            performance_df["model_type"] == "benchmark"
        ].sort_values("avg_mae")
        for _, row in benchmark_performance.iterrows():
            ...
            report_lines.append(
                f"**{model.upper()}**: MAE={mae:.3f} ug/m³, R²={r2:.3f}"
            )

        report_lines.extend(["", "## POLLUTANT-SPECIFIC ANALYSIS", ""])
        ...
        for pollutant in self.pollutants:
            ...
            for _, model_row in pollutant_data.iterrows():
                ...
                report_lines.append(
                    f"  {marker} {model_row['model'].replace('_', ' ').title()}: {model_row['mae']:.3f} ug/m³"
                )
            report_lines.append("")
        ...
        best_improvement = max(
            best_overall["cams_improvement_pct"], best_overall["noaa_improvement_pct"]
        )
