...
class WalkForwardValidator:
    ...
    def __init__(self, output_dir: str = "data/analysis/stage4_walk_forward_validation"):
    ...
    def create_realistic_dataset(self, city_info: Dict, continent: str) -> pd.DataFrame:
        ...
        for pollutant in self.pollutants:
            ...
            data[f"{pollutant}_source1"] = data[pollutant] + np.random.normal(0, data[pollutant] * 0.05, total_days)
            # Secondary source (medium quality)  
            data[f"{pollutant}_source2"] = data[pollutant] + np.random.normal(0, data[pollutant] * 0.10, total_days)
            # Tertiary source (lower quality)
            data[f"{pollutant}_source3"] = data[pollutant] + np.random.normal(0, data[pollutant] * 0.15, total_days)
    ...
    def create_prediction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [
            ...
            # Temporal
            "day_of_year", "day_of_week", "month", "season", "is_holiday", "is_weekend",
            # Regional
            "dust_event", "wildfire_smoke", "heating_load", "transport_density",
            # Quality
            "data_quality_score", "source_confidence", "completeness"
        ]
        
        # Add pollutant source data (what models use as inputs)
        for pollutant in self.pollutants:
            feature_cols.extend([f"{pollutant}_source1", f"{pollutant}_source2", f"{pollutant}_source3"])
    ...
    def evaluate_continental_performance(self, continent: str) -> Dict[str, Any]:
        ...
        for city_idx in range(num_cities):
            ...
            city_results = self.walk_forward_validate_city(city_data, city_info, continent)
    ...
    def aggregate_continental_results(
        ...
        for model_name in self.models.keys():
            # Performance metrics
            r2_scores = [city["model_performance"][model_name]["r2_score"] for city in city_results]
            mae_scores = [city["model_performance"][model_name]["mae"] for city in city_results]
            rmse_scores = [city["model_performance"][model_name]["rmse"] for city in city_results]
            ...
            for city in city_results:
                if model_name in city["health_warnings"]:
                    ...
                    if "sensitive" in health_data:
                        sensitive_fn_rates.append(health_data["sensitive"]["false_negative_rate"])
                        total_sensitive_alerts += health_data["sensitive"]["total_alerts"]
                        total_sensitive_fn += health_data["sensitive"]["false_negatives"]
                    
                    if "general" in health_data:
                        general_fn_rates.append(health_data["general"]["false_negative_rate"])
            ...
            health_aggregates[model_name] = {
                "mean_sensitive_fn_rate": np.mean(sensitive_fn_rates) if sensitive_fn_rates else 0,
                "mean_general_fn_rate": np.mean(general_fn_rates) if general_fn_rates else 0,
                "overall_sensitive_fn_rate": total_sensitive_fn / total_sensitive_alerts if total_sensitive_alerts > 0 else 0,
                "overall_general_fn_rate": total_general_fn / total_general_alerts if total_general_alerts > 0 else 0,
                "total_sensitive_alerts": total_sensitive_alerts,
                "total_general_alerts": total_general_alerts
            }
        
        # Best performing model
        best_model = max(model_aggregates.keys(), key=lambda m: model_aggregates[m]["mean_r2"])
    ...
    def evaluate_global_system(self) -> Dict[str, Any]:
        ...
        for continent in self.continental_patterns.keys():
            ...
            log.info(f"{continent.title()}: Best Model = {best_model} (R² = {best_r2:.3f})")
    ...
    def create_global_summary(self, continental_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for model_name in self.models.keys():
            ...
            for continent_data in continental_results.values():
                ...
                for city_result in continent_data["city_results"]:
                    all_r2s.append(city_result["model_performance"][model_name]["r2_score"])
                    all_maes.append(city_result["model_performance"][model_name]["mae"])
                
                total_production_cities += model_data["cities_above_r2_80"]
                total_sensitive_fn += health_data["total_sensitive_alerts"] - health_data.get("total_sensitive_alerts", 0)
            ...
            global_health_performance[model_name] = {
                "global_sensitive_fn_rate": total_sensitive_fn / total_sensitive_alerts if total_sensitive_alerts > 0 else 0,
                "total_sensitive_alerts": total_sensitive_alerts
            }
        ...
        best_health_fn_rate = global_health_performance[best_health_model]["global_sensitive_fn_rate"]
        production_cities = global_model_performance[best_accuracy_model]["production_ready_cities"]
        ...
        success_criteria = {
            ...
            "all_criteria_met": (best_accuracy >= 0.75) and (best_health_fn_rate <= 0.10) and (production_cities >= 60)
        }
    ...
    def save_validation_results(self, results: Dict[str, Any]) -> None:
        ...
        for model_name in self.models.keys():
            summary_data.append({
                ...
                "production_ready_cities": global_perf[model_name]["production_ready_cities"],
                "global_success_rate": global_perf[model_name]["global_success_rate"],
                "sensitive_false_negative_rate": global_health[model_name]["global_sensitive_fn_rate"],
                "health_criteria_met": global_health[model_name]["global_sensitive_fn_rate"] <= 0.10
            })
...
def main():
    ...
    print(f"• Health Warnings (≤10% FN): {'✅' if success['health_warnings_met'] else '❌'}")
    print(f"• Production Cities (≥60): {'✅' if success['production_cities_met'] else '❌'}")
