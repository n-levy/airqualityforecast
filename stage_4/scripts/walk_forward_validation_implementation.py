...
class WalkForwardValidator:
    ...
    def load_and_prepare_data(self):
        ...
        logger.info(f"Training data: {len(train_data):,} records ({train_data['datetime'].min()} to {train_data['datetime'].max()})")
        logger.info(f"Test data: {len(test_data):,} records ({test_data['datetime'].min()} to {test_data['datetime'].max()})")
        
        # Prepare feature columns (exclude predictions and actuals)
        exclude_cols = ['city', 'datetime', 'date'] + self.actual_cols + self.cams_cols + self.noaa_cols
        ...
        for col in potential_features:
            ...
            elif col == 'week_position':  # Handle known categorical that should be encoded
                continue  # Skip for now, could be encoded later
        
        logger.info(f"Using {len(self.feature_cols)} numeric features for ML models")
        logger.info(f"Excluded categorical features: {set(potential_features) - set(self.feature_cols)}")
    ...
    def generate_comparison_report(self, metrics_df):
        ...
        for model in avg_metrics.index:
            ...
            report_lines.append(f"**{model.upper()}**: MAE={mae:.3f}, R²={r2:.3f}, Correlation={corr:.3f}")
        ...
        # Performance by pollutant
        for pollutant in self.pollutants:
            pollutant_metrics = metrics_df[metrics_df['pollutant'] == pollutant].sort_values('mae')
            report_lines.extend([
                f"**{pollutant.upper()}**:",
                ""
            ])
            ...
            for _, row in pollutant_metrics.iterrows():
                report_lines.append(f"  • {row['model']}: MAE={row['mae']:.3f}, R²={row['r2']:.3f}")
            
            report_lines.append("")
        
        # Calculate improvements
        report_lines.extend([
            "### Model Improvements Over Benchmarks:",
            ""
        ])
